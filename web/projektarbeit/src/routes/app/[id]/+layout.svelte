<script lang="ts">
	...
	const links = $derived({
		general: [
			{
				title: 'Klausurenplan',
				url: `/app/${data.sidebarData?.routeId}/schedule`,
				icon: CalendarRange
			},
			{
				title: 'Graph',
				url: `/app/${data.sidebarData?.routeId}/graph`,
				icon: GitGraph
			}
		],
		settings: [
			{
				title: 'Allgemein',
				url: `/app/${data.sidebarData?.routeId}/settings/general`
			},
			{
				title: 'Kursliste',
				url: `/app/${data.sidebarData?.routeId}/settings/course-list`
			},
			{
				title: 'Kurse',
				url: `/app/${data.sidebarData?.routeId}/settings/courses`
			},
			{
				title: 'Zeitraum',
				url: `/app/${data.sidebarData?.routeId}/settings/timeperiod`
			}
		]
	});
	...
	let breadcrumbItems = $derived(
		pathSegments.map((segment, index) => {
			...
			const label =
				segment === data.sidebarData?.routeId
